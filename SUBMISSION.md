...
*Live watch test 2026-09-22:* two bounded runs (`--interval 5 --cycles 2` and `--interval 15 --cycles 2`) were executed against the real public RPC. Both established the baseline signature successfully; both then hit the public endpoint's `getParsedTransactions` rate limit ("Too many requests for a specific RPC call") on the fresh-signature fetch. Watch mode behaved exactly as designed under the throttle: each cycle printed a labeled `RPC error … retrying next cycle`, continued polling, and exited with a clean zero-transfer summary and exit 0. (An earlier accidental unbounded run in the same session showed the same throttling pattern.) Takeaway: the polling, alert, and error-handling paths are live-verified; full screening throughput in watch mode needs a dedicated RPC endpoint rather than the shared public one. "24/7" uptime language stays removed pending a live soak test on such an endpoint.*
